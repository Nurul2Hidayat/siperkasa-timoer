	...
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h5><i class="fa fa-table"></i> Tipe Aset</h5>
			</header>
			...
			<form id="add_data_form" onsubmit="return false">
				<div class="modal-body">
					<div class="form-group row">
						...
						<div class="col-sm-6">
							<select data-plugin-selectTwo class="form-control populate" id="g_aset_add" name="g_aset" class="form-control">
					...
					<div class="form-group row">
						...
						<div class="col-sm-10">
							<input id="g_nama_add" maxlength="54" name="g_nama" type="text" class="form-control" placeholder="Nama Tipe">
						</div>
					</div>
					<div class="form-group row">
			...
			<form id="edit_data_form" onsubmit="return false">
				<div class="modal-body">
					<div class="form-group row">
						<label class="col-sm-2 col-form-label">Aset</label>
						<div class="col-sm-10">
							<select data-plugin-selectTwo class="form-control populate" id="g_aset_edit" name="g_aset" class="form-control">
...
<script>
	...
	function get_data(){
		$.ajax({
            url: 'master/tipe_aset_data.php?f=get_data',
            type: 'POST',
			dataType: 'json',
	...
	function edit_data(id, jenis_id){
		$.ajax({
			url		: 'master/tipe_aset_data.php?f=get_data_edit',
			type	: 'POST',
			data	: {'gid':id},
			dataType: 'json',
			success	: function(data){
				get_aset(data.aset_id, 'edit');
				//$('#g_aset_edit').select2('val', data.aset_id);
				$('#g_id_edit').val(data.id);
				$('#g_nama_edit').val(data.nm_tipe);
				$('#g_deskripsi_edit').val(data.ket);
	...
	function add_data(){
		...
		$.ajax({
			url		: 'master/tipe_aset_data.php?f=add_data',
			type	: 'POST',
			data	: formData,
			dataType: 'json',
			processData: false,
	...
	function edit_data_proses(){
		...
		$.ajax({
			url		: 'master/tipe_aset_data.php?f=edit_data_proses',
			type	: 'POST',
			data	: formData,
			dataType: 'json',
			processData: false,
	...
	function hapus_data(id){
		...
		if(cc == true){
			$.ajax({
				url		: 'master/tipe_aset_data.php?f=hapus_data&gid='+id,
				type	: 'get',
				contentType	: false,
				processData : false,
				success	: function(){
