	...
	function get_data(){
		...
		while($data = mysql_fetch_array($q)){
			...
			if($_SESSION['group'] == 1){
				$btn = "<td>
						<a href='#'>
							<button type='button' class='btn btn-success btn-xs' onclick='edit_data($data[permintaan_id], $data[jenis_id])'>
								<i class='fa fa-pencil'></i>
							</button>
						</a>
						<a href='#'>
							<button type='button' class='btn btn-danger btn-xs' onclick='hapus_data($data[permintaan_id])'>
				...
				$btn = "<td>
						<a href='#'>
							<button type='button' class='btn btn-success btn-xs' onclick='action($data[permintaan_id], $data[jenis_id], $data[aset_id])'>
...
Terimakasih";
			...
			if($par['val'] == 14){
				$confirm_token	= base64_encode(base64_encode($par['id'].'___confirm'));
$link ="
Mohon klik link dibawah ini untuk konfirmasi permintaan sudah selesai

http://".$_SERVER['SERVER_NAME']."/it/scan_qrcode/confirm.php?gid=$confirm_token

Terimakasih";
$data['nm_status'] = 'Selesai';
			}
